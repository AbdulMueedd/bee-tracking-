...
def run_pseudo_labeling(splits_dir, label_pct, tau, rounds, epochs, model_base):
    ...
    n_labeled_initial = len(list((work_dir / "images").glob("*.png")) +
                             list((work_dir / "images").glob("*")))
    print(f"Starting pseudo-labeling: {label_pct}% labeled ({n_labeled_initial} frames)")
    print(f"Unlabeled pool: {len(list((unlabeled_dir / 'images').glob('*.jpg')))} frames")
    ...
    for round_num in range(rounds + 1):
        ...
        # ── Train ──
        print(f"{'='*60}")
        print(f"Round {round_num}: Training on {len(list((work_dir / 'images').glob('*.jpg')))} frames")
        print(f"{'='*60}")
        ...
        best_model_path = round_dir / "train" / "weights" / "best.pt"
        if not best_model_path.exists():
            best_model_path = round_dir / "train" / "weights" / "last.pt"
...
def _generate_pseudo_labels(model_path, unlabeled_dir, work_dir, tau):
    ...
    unlabeled_imgs = sorted((unlabeled_dir / "images").glob("*"))
    n_added = 0
    ...
    already_added = set(f.stem for f in work_img_dir.glob("*"))
    ...
    for img_path in unlabeled_imgs:
        ...
        # Check if ALL detections in this frame are above threshold
        confs = boxes.conf.cpu().numpy()
        if confs.min() < tau:
            # At least one detection is below threshold — skip frame
            # (conservative: only add frames where model is confident about everything)
            # Alternative: only keep boxes above tau (uncomment below)
            pass
